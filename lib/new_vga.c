...
/*
static uint16_t *LineBuffer1=(uint16_t *)0x20000000; // LineBuffer1[LINE_LENGTH]; <-- NO ?, would be in CCM so out of reach of DMA !
static uint16_t *LineBuffer2=(uint16_t *)0x20000000+1024;//!pixels !  +LINE_LENGTH*sizeof(uint16_t)+2*MARGIN; // LineBuffer2[LINE_LENGTH];
*/
...
static void HSYNCHandler()
{
	...
	if(vga_line<480)
	{
		...
		{
			vga_frame++; 
			// TODO : VBlank interrupt. waiting for vga_frame can be enough.
		}
		else if(vga_line==490)
		{
			//GPIOA->BSRRH|=(1<<0); // lower VSync vga_line
			GPIOC->BSRRH|=(1<<11); // lower VSync vga_line // THIS IS NOT REAL BITBOX !! only prototype
		}
		else if(vga_line==492)
		{
			//GPIOA->BSRRL|=(1<<0); // raise VSync vga_line
			GPIOC->BSRRL|=(1<<11); // raise VSync vga_line
		}		
		else if(vga_line==524)
		{
			#ifdef AUDIO
			audio_frame();
			#endif 
