...
- displaying other buffer updating vga_line, vga_frame
...
uint16_t *draw_buffer = mybuffer; // volatile ?
...
void set_mode(int width, int height, bool fullscreen)
{
    screen_width = width;
...
int init(bool fullscreen)
{
    ...
    set_mode(640,480, fullscreen);
    audio_init();
    joy_init();

    printf("screen is now %dx%d %s\n",screen_width,screen_height, fullscreen ? "(fullscreen)" : "");
    return 0;
}
...
int main ( int argc, char** argv )
{
    ...
    gamepad1 = 0; // all up
    if (init(fullscreen)) return 1;
    game_init();


    // now start sound (after init)
    SDL_PauseAudio(0);
